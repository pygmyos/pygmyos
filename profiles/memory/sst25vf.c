...
u8 sst25vfVOpen( PYGMYSPIPORT **pygmySPI, u64 ulAddress, va_list vaList )
{
    ...
    if( ptrSPI ){
        *pygmySPI = ptrSPI;
        //putstr( "\rPort: " );
        print( COM3, "\rPort: %d", ptrSPI );
        //putIntUSART3( ptrSPI );
        //putstr( "\rConfiguring Port" );
        print( COM3, "\rConfiguring Port" );
        //putstr( "\r\tCS: " ); putstr( convertPinToString( ucCS ) ); putstr( " " ); putIntUSART3( ucCS ); 
        print( COM3, "\r\tCS: %s %d", convertPinToString( ucCS ), ucCS );
        //putstr( "\r\tSCK: " ); putstr( convertPinToString( ucSCK ) ); putstr( " " ); putIntUSART3( ucSCK );
        print( COM3, "\r\tSCK: %s %d", convertPinToString( ucSCK ), ucSCK );
        //putstr( "\r\tMISO: " ); putstr( convertPinToString( ucMISO ) ); putstr( " " ); putIntUSART3( ucMISO );
        print( COM3, "\r\tMISO: %s %d", convertPinToString( ucMISO ), ucMISO );
        //putstr( "\r\tMOSI: " ); putstr( convertPinToString( ucMOSI ) ); putstr( " " ); putIntUSART3( ucMOSI );
        print( COM3, "\r\tMOSI: %s %d", convertPinToString( ucMOSI ), ucMOSI );
        spiConfig( ptrSPI, ucCS, ucSCK, ucMISO, ucMOSI, SPILONGADDRESS|SPIDUMMYONREAD );
        //putstr( "\rUnlocking Flash" );
        print( COM3, "\rUnlocking Flash" );
...
u8 sst25vfDesc( PYGMYSPIPORT *pygmySPI, PYGMYMEMDESC *pygmyDesc )
{
    ...
    pygmySPI->PortCS->BSRR = pygmySPI->PinCS;
    //putstr( "\rCapacity: " ); putIntUSART3( pygmyDesc->Capacity );
    print( COM3, "\rCapacity: %d", pygmyDesc->Capacity );
        //putstr( "\rSectors: " ); putIntUSART3( pygmyDesc->Sectors );
        print( COM3, "\rSectors: %d", pygmyDesc->Sectors );
        //putstr( "\rSectorSize: " ); putIntUSART3( pygmyDesc->SectorSize );
        print( COM3, "\rSectorSize: %d", pygmyDesc->SectorSize );
        //putstr( "\rAttributes: " ); putIntUSART3( pygmyDesc->Attributes );
        print( COM3, "\rAttributes: %d", pygmyDesc->Attributes );
        //putstr( "\rManufacturer: " ); putIntUSART3( pygmyDesc->Manufacturer );
        print( COM3, "\rManufacturer: %d", pygmyDesc->Manufacturer );
        //putstr( "\rType: " ); putIntUSART3( pygmyDesc->Type ); 
